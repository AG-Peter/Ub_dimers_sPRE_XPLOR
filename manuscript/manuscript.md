...
MD simulations are a valuable tool to gain insights into a protein's structure and dynamics. The atomistic
resolution paired with the time steps in the range of pico seconds have enabled a closer look into the dynamic process of protein folding and unfolding. Compared to other methods which yield time-averaged results (please exchange for structure/... for more general audience.what is the result of md), MD can be used as a tool to identify specific sub-states and to predict transition paths between such states. However, reaching such conclusions requires large datasets that can be difficult to obtain and handle. System sizes and the associated number of atoms scale with r^3 . Recognizing the many opportunities of all-atom MD, a major obstacle is that folding of some rigid proteins might occur on such long timescales that all-atom simulations might not be feasible due to the mentioned drawbacks. In this manuscript we present a method to combine a limited set of all-atom MD data with a large dataset of coarse-grained MD data and relate then to solvent paramagnetic resonance enhancement (sPRE) spectroscopy. Three linked di-ubiquitin proteins were used as test system for this new method.
...
MD data at coarse-grained resolution was taken from [@berg2018towards]. This dataset comprised the ubiquitination sites K6, K29 and K33 where every ubiquitination is comprised of 10 replicas of 10 ms each. The MARTINI v2.2 force field was applied to the non-bonded interaction, whereas bonded interactions have been modeled using the IDEN method. These forcefields had to be altered to accommodate the isopeptide bond between the proximal subunit's lysine and the distal subunit's C-terminal glycine (Figure \ref{K6_CG}). The CG data of all 3 ubiquitylation sites encompass approximately 300 ms.
...
These simulations were used as input-data for a dimensionality reduction algorithm called sketch-map. Here, the high-dimensional phase space of the protein was projected into 2D. The similarity of any two conformations was represented in the distance in sketch-space. Naturally, basins of higher density start to appear for statistically significant conformation sub-states of low free energy. Susequently, new all-atom simulations were started from both the center of these basin (4 simulations per ubiquitination site with 10 ns each) and from randomly chosen points in the vicinity of the basin (40 simulations per ubiquitination site with 3 ns each) by using Martini's backward script paired with energy minimization to settle initial high-energy structure.

**Extended and rotamer atomistic data**

Additionally to the back-mapped all atom simulations, further atomistic data were taken from Berg et al. The starting structures of these simulations were created by slightly altering the position between the proximal and distal subunit of the di-ubiquitnated protein. As a reference for the monomer, a 1UBQ crystal structure from the protein database was used [@vijay1987structure]. For every ubiquitination site, 12 simulations with 50 ns each were available. The all-atom simulations employed the GROMOS54a7 forcefield which was adapted to accommodate for the isopeptide bond, which was parametrized from regular peptide bonds. The same force
field was also used for the rotamer simulations which have been carried out to complement the dataset of all-atom simulations. Here, the different starting structures were created by rotating the sidechain chi3 angle of the ubiquitinated  lysine residue in 40 degree steps. The rotamer simulations were intended to accelerate the 
exploration of the phase  pace available to the diubiquitin proteins. The data encompass 9 simulations per 
ubiquitination site with approximately 50 ns each.%this sentence makes no sense. Summarizing all simulation data, a single ubiquitination site was characterized by 100 ms of CG data and 1.21 ms of AA data.%introduce abbreviations 
...
subunit right, distal left. The residues of the isopeptide bond (LYQ and GLQ) are colored in orange.

## Methods

Generally, the analysis steps were performed as follows:
1. Extract high-dimensional collective variables
2. Project %%data into 2D space using Encodermap
3. Use HDBSCAN to extract statistically-significant sub-states of ensemble
4. Calculate sPRE values for the all-atom conformations using XPLOR NIH (task is parallelized due to significant%%)
...
Collective variables represent a type of data that is somehow aligned with the raw xyz positional data of a molecular dynamics trajectory. Oftentimes, CVs are used to visualize complex dynamic processes. %intro?One such example might be the simplification of receptor-ligand docking processes by using a receptor-ligand distance as a collective variable. The torsion angles of the backbone are another widely used example of collective variables. From the $\phi$ and $\psi$ dihedral angles a ramachandran plot can be created to visualize and simplify the complex structure of proteins. In this paper we faced the challenge to unify the CG and AA data, meaning CVs needed to be identical for both types of MD data. Torsional angles could not be calculated from CG data as the backbone N C and CA atoms are unified within one CG bead. Distance matrices between all CA atoms would yield a ${15\choose2} = 11476$ dimensional dataset. We chose to use the approach also used by Berg at al. employing residue-wise minimal distances, which makes the input data 152-dimensional.
%this is a cluster of sentences,but misses a red line
...
three cost functions: %why did you do this?remember audience is stupid  : this is necessary to...
...
The  resulting 2D projection could then be used as input to HDBSCAN clustering.
...
and especially the low-dimensional projections of such [@mcinnes2017hdbscan]. Hoever/nonetheless/our project is better/more challenging...
After the 2D projections were obtained for every ubiquitination site, the xy-values were fed into HDBSCAN using the 'leaf' algorithm, with a minimal cluster size of 2500, resulting in 13, 18, and 24 selected clusters for K6, K29, and K33, respectively. The clusters are enumerated based on the number of included points/conformations, with cluster 0 being the largest cluster. Another notable point is that HDBSCAN offers a great advantage by distinguishing between clusters and noise. Noise - speaking from an ensemble viewpoint - is comprised of unique structures that carry no significance % and thus need to be disregarded for the subsequent analysis. 
