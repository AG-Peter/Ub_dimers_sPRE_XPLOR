...
The high-dimensional dataset consists of the 152 residue-wise minimal distances for each MD frame (CG or atomistic).
The approximately XXXX frames per ubiquitination site, made it unfeasable to employ non-machine-learning dimensionality
algorithms. They could not cope with the number of data (for example: multidimensional-scaling would require a
$n_{frames} \times n_{frames}$ distance matrix, which would require ubnfeasable amounts of system memory. Sketch-map
would require to pre-select points and reduce the amount of data actually used in dimensionality reduction).
We used Encodermap's auto-encoder neural network to retrieve a dimensionally reduced representation of all 
